...
export async function joinSession(req, res) {
    try {
        ...
        if (session.status !== "active") {
            return res.status(400).json({ message: "Can't join a completed session" })
        }
        if (session.host.toString === userId.toString()) {
            return res.status(400).json({ message: "Host can't join their own session as participant" })
        }
        ...
        console.error("Error in joiSessions controller : ", error.message)
        res.status(500).json({ message: "Internal Server Error" })
    }
}
